...
import val.ValException.NotValidException;
import val.assetexchange.AssetExchange;
import val.at.AT_Constants;
import val.at.AT_Controller;
import val.at.AT_Exception;
import val.fluxcapacitor.FeatureToggle;
import val.fluxcapacitor.FluxCapacitor;
import val.services.*;
import val.transactionduplicates.TransactionDuplicationKey;
...
public abstract class TransactionType {
    ...
    public abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion)
            throws ValException.NotValidException;

    abstract Attachment.AbstractAttachment parseAttachment(JsonObject attachmentData)
            throws ValException.NotValidException;

    abstract void validateAttachment(Transaction transaction) throws ValException.ValidationException;
...
//                    transaction.getAmountNQT() - transaction.getFeeNQT());//unpledge提取直接到账
            accountService.addToBalanceNQT(senderAccount, -transaction.getFeeNQT());
            accountService.addToBalanceAndUnconfirmedBalanceNQT(senderAccount, transaction.getAmountNQT());
    ...
    public void parseAppendices(Transaction.Builder builder, int flags, byte version, ByteBuffer buffer)
            throws ValException.ValidationException {
        int position = 1;
    ...
    public static abstract class Payment extends TransactionType {
        ...
        public static final TransactionType ORDINARY = new Payment() {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
                    throw new ValException.NotValidException("Invalid ordinary payment");
                }
            }
        ...
        public static final TransactionType MULTI_OUT = new Payment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                if (!fluxCapacitor.isActive(FeatureToggle.PRE_DYMAXION, transaction.getHeight())) {
                    throw new ValException.NotCurrentlyValidException(
                ...
                if (amountNQT <= 0 || amountNQT >= Constants.MAX_BALANCE_NQT || amountNQT != transaction.getAmountNQT()
                    ...
                    throw new ValException.NotValidException("Invalid multi out payment");
                }
            }
        ...
        public static final TransactionType PAYMENT_TRANSACTIONS_GENESIS = new Payment() {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                throw new ValException.NotValidException("Genesis transaction never validates");
            }
        ...
        public static final TransactionType PAYMENT_TRANSACTIONS_PLEDGE = new Payment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
                    throw new ValException.NotValidException("Invalid pledge payment");
                }
                ...
                if (pledge != null && pledge.getPledgeTotal() > 0
                    ...
                    throw new ValException.NotValidException("Invalid mine pool account");
                }
        ...
        public static final TransactionType PAYMENT_TRANSACTIONS_UNPLEDGE = new Payment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
                    throw new ValException.NotValidException("Invalid unpledge payment");
                }

                Account.Pledges pledge = accountService.getAccountPledge(transaction.getSenderId());
                if (pledge == null) {
                    throw new ValException.NotValidException("no pledge for account: " + transaction.getSenderId());
                } else if (pledge.pledgeTotal < transaction.getAmountNQT()) {
                    // || pledge.getRecipID() != transaction.getRecipientId()) {
                    throw new ValException.NotValidException("Invalid unpledge amount < pledgeTotal");
                }
                Account poolAccount = accountService.getAccount(pledge.getRecipID());
                if (poolAccount == null) {
                    throw new ValException.NotValidException("no account info for account: " + pledge.getRecipID());
                } else if (poolAccount.getTotalPledged() < transaction.getAmountNQT()) {
                    throw new ValException.NotValidException(
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (!TextUtils.isInAlphabet(attachment.getAliasName())) {
                    throw new ValException.NotValidException("Invalid alias name: " + attachment.getAliasName());
                }
        ...
        public static final TransactionType ALIAS_SELL = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                if (!fluxCapacitor.isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
                    throw new ValException.NotYetEnabledException(
                ...
                if (aliasName == null || aliasName.isEmpty()) {
                    throw new ValException.NotValidException("Missing alias name");
                }
                long priceNQT = attachment.getPriceNQT();
                if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
                    throw new ValException.NotValidException("Invalid alias sell price: " + priceNQT);
                }
                if (priceNQT == 0) {
                    if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
                        ...
                        throw new ValException.NotValidException("Missing alias transfer recipient");
                    }
                }
        ...
        public static final TransactionType ALIAS_BUY = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (offer == null) {
                    throw new ValException.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
                }
                if (transaction.getAmountNQT() < offer.getPriceNQT()) {
                    String msg = "Price is too low for: " + aliasName + " (" + transaction.getAmountNQT() + " < "
                            + offer.getPriceNQT() + ")";
                    throw new ValException.NotCurrentlyValidException(msg);
                }
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (!TextUtils.isInAlphabet(attachment.getName())) {
                    throw new ValException.NotValidException("Invalid asset name: " + attachment.getName());
                }
            }
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0
                    ...
                    throw new ValException.NotValidException("Invalid asset transfer amount or comment: "
                            + JSON.toJsonString(attachment.getJsonObject()));
                }
                ...
                if (attachment.getQuantityQNT() <= 0
                    ...
                    throw new ValException.NotValidException("Invalid asset transfer asset or quantity: "
                            + JSON.toJsonString(attachment.getJsonObject()));
                }
                if (asset == null) {
                    throw new ValException.NotCurrentlyValidException(
        ...
        abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
            ...
            @Override
            final void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (attachment.getQuantityQNT() <= 0
                        || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
                    throw new ValException.NotValidException("Invalid asset order placement asset or quantity: "
                            + JSON.toJsonString(attachment.getJsonObject()));
                }
                if (asset == null) {
                    throw new ValException.NotCurrentlyValidException(
    ...
    public static abstract class DigitalGoods extends TransactionType {
        ...
        public static final TransactionType LISTING = new DigitalGoods() {
            ...
            @Override
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                Attachment.DigitalGoodsListing attachment = (Attachment.DigitalGoodsListing) transaction
                        .getAttachment();
        ...
        public static final TransactionType DELISTING = new DigitalGoods() {
            ...
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (goods == null || goods.isDelisted()) {
                    throw new ValException.NotCurrentlyValidException("Goods "
        ...
        public static final TransactionType PURCHASE = new DigitalGoods() {
            ...
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (transaction.getEncryptedMessage() != null && !transaction.getEncryptedMessage().isText()) {
                    throw new ValException.NotValidException("Only text encrypted messages allowed");
                }
                if (goods == null || goods.isDelisted()) {
                    throw new ValException.NotCurrentlyValidException("Goods "
                            + Convert.toUnsignedLong(attachment.getGoodsId()) + "not yet listed or already delisted");
        ...
        public static final TransactionType FEEDBACK = new DigitalGoods() {
            ...
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
                    throw new ValException.NotValidException("Missing feedback message");
                }
                if (transaction.getEncryptedMessage() != null && !transaction.getEncryptedMessage().isText()) {
                    throw new ValException.NotValidException("Only text encrypted messages allowed");
                }
                if (transaction.getMessage() != null && !transaction.getMessage().isText()) {
                    throw new ValException.NotValidException("Only text public messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null) {
                    throw new ValException.NotCurrentlyValidException(
        ...
        final void validateAttachment(Transaction transaction) throws ValException.ValidationException {
            ...
            if (transaction.getAmountNQT() != 0) {
                throw new ValException.NotValidException("Invalid digital goods transaction");
            }
            doValidateAttachment(transaction);
        }

        abstract void doValidateAttachment(Transaction transaction) throws ValException.ValidationException;
    ...
    public static abstract class ChainMining extends TransactionType {
        ...
        public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new ChainMining() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (sender == null) {
                    throw new ValException.NotCurrentlyValidException("Sender not yet known ?!");
                }
                ...
                if (!ValNetwork.getFluxCapacitor().isActive(FeatureToggle.REWARD_RECIPIENT_ENABLE, height)) {
                    throw new ValException.NotCurrentlyValidException(
                            "Reward recipient assignment not allowed before block " + ValNetwork.getFluxCapacitor()
    ...
    public static abstract class AdvancedPayment extends TransactionType {
        ...
        public static final TransactionType ESCROW_CREATION = new AdvancedPayment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (transaction.getSenderId() == transaction.getRecipientId()) {
                    throw new ValException.NotValidException("Escrow must have different sender and recipient");
                }
                totalAmountNQT = Convert.safeAdd(totalAmountNQT, attachment.getTotalSigners() * Constants.ONE_COIN);
                if (transaction.getAmountNQT() != 0) {
                    throw new ValException.NotValidException("Transaction sent amount must be 0 for escrow");
                }
                if (totalAmountNQT.compareTo(0L) < 0 || totalAmountNQT.compareTo(Constants.MAX_BALANCE_NQT) > 0) {
                    throw new ValException.NotValidException("Invalid escrow creation amount");
                }
                if (transaction.getFeeNQT() < Constants.ONE_COIN) {
                    throw new ValException.NotValidException("Escrow transaction must have a fee at least 1 vol");
                }
                if (attachment.getRequiredSigners() < 1 || attachment.getRequiredSigners() > 10) {
                    throw new ValException.NotValidException("Escrow required signers much be 1 - 10");
                }
                if (attachment.getRequiredSigners() > attachment.getTotalSigners()) {
                    throw new ValException.NotValidException("Cannot have more required than signers on escrow");
                }
                if (attachment.getTotalSigners() < 1 || attachment.getTotalSigners() > 10) {
                    throw new ValException.NotValidException("Escrow transaction requires 1 - 10 signers");
                }
                if (attachment.getDeadline() < 1 || attachment.getDeadline() > 7776000) { // max
                    // deadline
                    // 3
                    // months
                    throw new ValException.NotValidException("Escrow deadline must be 1 - 7776000 seconds");
                }
                if (attachment.getDeadlineAction() == null
                        || attachment.getDeadlineAction() == Escrow.DecisionType.UNDECIDED) {
                    throw new ValException.NotValidException("Invalid deadline action for escrow");
                }
                if (attachment.getSigners().contains(transaction.getSenderId())
                        || attachment.getSigners().contains(transaction.getRecipientId())) {
                    throw new ValException.NotValidException("Escrow sender and recipient cannot be signers");
                }
                if (!escrowService.isEnabled()) {
                    throw new ValException.NotYetEnabledException("Escrow not yet enabled");
                }
            }
        ...
        public static final TransactionType ESCROW_SIGN = new AdvancedPayment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                Attachment.AdvancedPaymentEscrowSign attachment = (Attachment.AdvancedPaymentEscrowSign) transaction
                        .getAttachment();
                if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_COIN) {
                    throw new ValException.NotValidException("Escrow signing must have amount 0 and fee of 1");
                }
                if (attachment.getEscrowId() == null || attachment.getDecision() == null) {
                    throw new ValException.NotValidException("Escrow signing requires escrow id and decision set");
                }
                Escrow escrow = escrowService.getEscrowTransaction(attachment.getEscrowId());
                if (escrow == null) {
                    throw new ValException.NotValidException("Escrow transaction not found");
                }
                if (!escrowService.isIdSigner(transaction.getSenderId(), escrow)
                    ...
                    throw new ValException.NotValidException("Sender is not a participant in specified escrow");
                }
                if (escrow.getSenderId().equals(transaction.getSenderId())
                        && attachment.getDecision() != Escrow.DecisionType.RELEASE) {
                    throw new ValException.NotValidException("Escrow sender can only release");
                }
                if (escrow.getRecipientId().equals(transaction.getSenderId())
                        && attachment.getDecision() != Escrow.DecisionType.REFUND) {
                    throw new ValException.NotValidException("Escrow recipient can only refund");
                }
                if (!escrowService.isEnabled()) {
                    throw new ValException.NotYetEnabledException("Escrow not yet enabled");
                }
        ...
        public static final TransactionType ESCROW_RESULT = new AdvancedPayment() {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                throw new ValException.NotValidException("Escrow result never validates");
            }
        ...
        public static final TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (attachment.getFrequency() == null
                    ...
                    throw new ValException.NotValidException("Invalid subscription frequency");
                }
                if (transaction.getAmountNQT() < Constants.ONE_COIN
                        || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
                    throw new ValException.NotValidException("Subscriptions must be at least one vol");
                }
                if (transaction.getSenderId() == transaction.getRecipientId()) {
                    throw new ValException.NotValidException("Cannot create subscription to same address");
                }
                if (!subscriptionService.isEnabled()) {
                    throw new ValException.NotYetEnabledException("Subscriptions not yet enabled");
                }
            }
        ...
        public static final TransactionType SUBSCRIPTION_CANCEL = new AdvancedPayment() {
            ...
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (attachment.getSubscriptionId() == null) {
                    throw new ValException.NotValidException("Subscription cancel must include subscription id");
                }
                ...
                if (!subscription.getSenderId().equals(transaction.getSenderId())
                    ...
                    throw new ValException.NotValidException("Subscription cancel can only be done by participants");
                }

                if (!subscriptionService.isEnabled()) {
                    throw new ValException.NotYetEnabledException("Subscription cancel not yet enabled");
                }
            }
        ...
        public static final TransactionType SUBSCRIPTION_PAYMENT = new AdvancedPayment() {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws ValException.ValidationException {
                throw new ValException.NotValidException("Subscription payment never validates");
            }
    ...
    public static abstract class AutomatedTransactions extends TransactionType {
        public static final TransactionType AUTOMATED_TRANSACTION_CREATION = new AutomatedTransactions() {
            ...
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                ...
                if (!fluxCapacitor.isActive(FeatureToggle.AUTOMATED_TRANSACTION_BLOCK,
                    ...
                    throw new ValException.NotYetEnabledException("Automated Transactions not yet enabled at height "
                            + blockchain.getLastBlock().getHeight());
                }
                if (transaction.getSignature() != null && accountService.getAccount(transaction.getId()) != null) {
                    ...
                    if (existingAccount.getPublicKey() != null
                        ...
                        throw new ValException.NotValidException("Account with id already exists");
                }
                ...
                try {
                    ...
                    throw new ValException.NotCurrentlyValidException("Invalid AT creation bytes", e);
                }
                ...
                if (transaction.getFeeNQT() < requiredFee) {
                    throw new ValException.NotValidException("Insufficient fee for AT creation. Minimum: "
                ...
                if (fluxCapacitor.isActive(FeatureToggle.AT_FIX_BLOCK_3)) {
                    if (attachment.getName().length() > Constants.MAX_AUTOMATED_TRANSACTION_NAME_LENGTH) {
                        throw new ValException.NotValidException("Name of automated transaction over size limit");
                    }
        ...
        public static final TransactionType AT_PAYMENT = new AutomatedTransactions() {
            ...
            void doValidateAttachment(Transaction transaction) throws ValException.ValidationException {
                 ...
                 */
                throw new ValException.NotValidException("AT payment never validates");
            }
        ...
        final void validateAttachment(Transaction transaction) throws ValException.ValidationException {
            if (transaction.getAmountNQT() != 0) {
                throw new ValException.NotValidException("Invalid automated transaction transaction");
            }
            doValidateAttachment(transaction);
        }
