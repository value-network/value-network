...
public abstract class VersionedEntitySqlTable<T> extends EntitySqlTable<T> implements VersionedEntityTable<T> {
    ...
    static void rollback(final String table, final TableImpl tableClass, final int height, final val.db.sql.DbKey.Factory dbKeyFactory) {
        ...
        try (DSLContext ctx = Db.getDSLContext()) {
            ...
            List<val.db.sql.DbKey> dbKeys = new ArrayList<>();
            try (ResultSet toDeleteResultset = selectForDeleteQuery.fetchResultSet()) {
                while (toDeleteResultset.next()) {
                    dbKeys.add((val.db.sql.DbKey) dbKeyFactory.newKey(toDeleteResultset));
                }
            }
            ...
            for (val.db.sql.DbKey dbKey : dbKeys) {
    ...
    static void trim(final String table, final TableImpl tableClass, final int height, final val.db.sql.DbKey.Factory dbKeyFactory) {
        if (!Db.isInTransaction()) {
            throw new IllegalStateException("Not in transaction");
        }
        ...
        try (ResultSet rs = selectMaxHeightQuery.fetchResultSet()) {
            ...
            while (rs.next()) {
                val.db.sql.DbKey dbKey = (val.db.sql.DbKey) dbKeyFactory.newKey(rs);
    ...
    public boolean delete(T t) {
        ...
        try (DSLContext ctx = Db.getDSLContext()) {
            ...
            countQuery.addConditions(tableClass.field("height", Integer.class).lt(ValNetwork.getBlockchain().getHeight()));
