...
public interface Appendix {
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        AbstractAppendix(int blockchainHeight) {
            ...
            this.version = (byte) (ValNetwork.getFluxCapacitor().isActive(FeatureToggle.DIGITAL_GOODS_STORE, blockchainHeight) ? 0 : 1);
        }
        ...
        public abstract void validate(Transaction transaction) throws ValException.ValidationException;
    ...
    class Message extends AbstractAppendix {
        ...
        public Message(ByteBuffer buffer, byte transactionVersion) throws ValException.NotValidException {
            ...
            if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new ValException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
        ...
        @Override
        public void validate(Transaction transaction) throws ValException.ValidationException {
            if (this.isText && transaction.getVersion() < 0) {
                throw new ValException.NotValidException("Text messages not yet enabled");
            }
            if (transaction.getVersion() < 0 && transaction.getAttachment() != Attachment.ARBITRARY_MESSAGE) {
                throw new ValException.NotValidException("Message attachments not enabled for version 0 transactions");
            }
            if (message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new ValException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws ValException.NotValidException {
            super(buffer, transactionVersion);
        ...
        public void validate(Transaction transaction) throws ValException.ValidationException {
            if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new ValException.NotValidException("Max encrypted message length exceeded");
            }
            if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                ...
                throw new ValException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
            }
        }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        public EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws ValException.ValidationException {
            super(buffer, transactionVersion);
        }
        ...
        public void validate(Transaction transaction) throws ValException.ValidationException {
            ...
            if (!transaction.getType().hasRecipient()) {
                throw new ValException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
            if (transaction.getVersion() < 0) {
                throw new ValException.NotValidException("Encrypted message attachments not enabled for version 0 transactions");
            }
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        public EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws ValException.ValidationException {
            super(buffer, transactionVersion);
        }
        ...
        public void validate(Transaction transaction) throws ValException.ValidationException {
            super.validate(transaction);
            if (transaction.getVersion() < 0) {
                throw new ValException.NotValidException("Encrypt-to-self message attachments not enabled for version 0 transactions");
            }
        }
    ...
    class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        public void validate(Transaction transaction) throws ValException.ValidationException {
            if (!transaction.getType().hasRecipient()) {
                throw new ValException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (publicKey.length != 32) {
                throw new ValException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new ValException.NotValidException("Announced public key does not match recipient accountId");
            }
            if (transaction.getVersion() < 0) {
                throw new ValException.NotValidException("Public key announcements not enabled for version 0 transactions");
            }
            ...
            if (recipientAccount != null && recipientAccount.getPublicKey() != null && !Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
                throw new ValException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
