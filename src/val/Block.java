...
public class Block {
    ...
    Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT,
          ...
          long nonce, byte[] blockATs, int height) throws ValException.ValidationException {

        if (payloadLength > ValNetwork.getFluxCapacitor().getInt(FluxInt.MAX_PAYLOAD_LENGTH, height) || payloadLength < 0) {
        ...
        if (transactions != null) {
            ...
            if (blockTransactions.get().size() > (ValNetwork.getFluxCapacitor().getInt(FluxInt.MAX_NUMBER_TRANSACTIONS, height))) {
                throw new ValException.NotValidException(
            ...
            for (Transaction transaction : this.blockTransactions.get()) {
                if (transaction.getId() <= previousId && previousId != 0) {
                    ...
                    throw new ValException.NotValidException("Block transactions are not sorted!");
                }
    ...
    public Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash, byte[] generatorPublicKey, byte[] generationSignature, byte[] blockSignature, byte[] previousBlockHash, BigInteger cumulativeDifficulty, long baseTarget,
                 long nextBlockId, int height, Long id, long nonce, byte[] blockATs) throws ValException.ValidationException {
    ...
    static Block parseBlock(JsonObject blockData, int height) throws ValException.ValidationException {
        try {
            ...
            for (JsonElement transactionData : transactionsData) {
                ...
                if (transaction.getSignature() != null) {
                    if (blockTransactions.put(transaction.getId(), transaction) != null) {
                        throw new ValException.NotValidException("Block contains duplicate transactions: " + transaction.getStringId());
                    }
                }
            }
    ...
    private TransactionDb transactionDb() {
        return ValNetwork.getDbs().getTransactionDb();
    }
